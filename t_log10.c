...
ATF_TC_BODY(test_log101, tc)
{
	...
	for (i = 0; i < N; i++)
		ATF_CHECK(fpcmp_equal(log10(ttable[i].x), ttable[i].y));
}
...
#ifdef	NAN
	ATF_CHECK(isnan(log10f(NAN)));
	ATF_CHECK(isnan(log10(NAN)));
	ATF_CHECK(isnan(log10l(NAN)));
#endif
