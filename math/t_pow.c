...
ATF_TC_BODY(test_pow42, tc)
{
	...
	ATF_FOR_LOOP(i, N, i++) {
		...
		fy = rand() % 2 ? fy : NAN;
#endif
		ATF_CHECK(fpreal_equalf(powf(1.0, fy), 1.0));
		...
		dy = rand() % 2 ? dy : NAN;
#endif
		ATF_CHECK(fpreal_equal(pow(1.0, dy), 1.0));
		...
		ldy = rand() % 2 ? ldy : NAN;
#endif
		ATF_CHECK(fpreal_equall(powl(1.0, ldy), 1.0));
#endif  /* HAVE_POWL */
	}
}
...
ATF_TC_BODY(test_pow43, tc)
{
	...
	ATF_FOR_LOOP(i, N, i++) {
		...
		fx = rand() % 2 ? fx : NAN;
#endif
		ATF_CHECK(fpreal_equalf(powf(fx,  0.0), 1.0));
		ATF_CHECK(fpreal_equalf(powf(fx, -0.0), 1.0));
		...
		dx = rand() % 2 ? dx : NAN;
#endif
		ATF_CHECK(fpreal_equal(pow(dx,  0.0), 1.0));
		ATF_CHECK(fpreal_equal(pow(dx, -0.0), 1.0));
		...
		ldx = rand() % 2 ? ldx : NAN;
#endif
		ATF_CHECK(fpreal_equal(powl(ldx,  0.0), 1.0));
		ATF_CHECK(fpreal_equal(powl(ldx, -0.0), 1.0));
#endif	/* HAVE_POWL */
	}
}
...
ATF_TC_BODY(test_pow44, tc)
{
	...
	ATF_FOR_LOOP(i, N, i++) {
		...
		/* float */
		ATF_PASS_OR_BREAK(fpreal_equalf(powf( 0.0, y),  0.0));
		ATF_PASS_OR_BREAK(fpreal_equalf(powf(-0.0, y), -0.0));

		/* double */
		ATF_PASS_OR_BREAK(fpreal_equal(pow( 0.0, y),  0.0));
		ATF_PASS_OR_BREAK(fpreal_equal(pow(-0.0, y), -0.0));

		/* long double */
#ifdef  HAVE_POWL
		ATF_PASS_OR_BREAK(fpreal_equall(powl( 0.0, y),  0.0));
		ATF_PASS_OR_BREAK(fpreal_equall(powl(-0.0, y), -0.0));
#endif
	}
}
...
ATF_TC_BODY(test_pow45, tc)
{
	...
	ATF_FOR_LOOP(i, N, i++) {
		...
		ATF_PASS_OR_BREAK(fpreal_equalf(powf( 0.0, fy), 0.0));
		ATF_PASS_OR_BREAK(fpreal_equalf(powf(-0.0, fy), 0.0));
		...
		ATF_PASS_OR_BREAK(fpreal_equal(pow( 0.0, dy), 0.0));
		ATF_PASS_OR_BREAK(fpreal_equal(pow(-0.0, dy), 0.0));
		...
		ATF_PASS_OR_BREAK(fpreal_equall(powl( 0.0, ldy), 0.0));
		ATF_PASS_OR_BREAK(fpreal_equall(powl(-0.0, ldy), 0.0));
#endif
	}
}
